      ...
      props: [
        {
          name: "columns",
          type: "KanbanColumn[]",
          description: "The board. Required and controlled only — the board never forks state, it renders what you pass and reports the next board through `onChange`.",
        },
        {
          name: "onChange",
        ...
        },
        {
          ...
          description: "Replaces the default header — the column title with a count badge.",
        },
        {
          name: "disabled",
          type: "boolean",
      ...
      props: [
        {
          name: "id",
          type: "string",
          description: "Drop target id, and the `toColumnId` `moveItem` takes. Must be a non-duplicate string — column and card ids share one id space.",
        },
        {
          name: "title",
          type: "string",
      ...
      props: [
        {
          name: "columns",
          type: "KanbanColumn[]",
          description: "The board to move within. Never mutated — neither it, its columns nor its items.",
        },
        {
          name: "itemId",
          type: "string",
        ...
        },
        {
          name: "→ returns",
          type: "KanbanColumn[]",
          description: "A new board with untouched columns kept by identity, so consumers can memo on them — or the input array itself (reference-equal) whenever the move would change nothing, which is how the board decides to skip `onChange`.",
        },
      ],
    },
    {
      ...
      props: [
        ...
        },
        {
          ...
          description: "dnd-kit's sensor state. The board is walked structurally through `context` — the `type`/`columnId` data every droppable here carries — instead of by rect geometry, which is what makes left/right and within-column up/down work across several `SortableContext`s.",
        },
        {
          name: "→ returns",
...
// Send a card to the top of another column — same helper the board uses on drop.
