...
export function AvatarCropper({ onUpload }: { onUpload: (file: Blob) => void }) {
  ...
  errorState:
    "Framing cannot go wrong, so the cropper never invalidates itself. Zoom is capped at 1–3×, the crop box stays inside the image, and a garbage area is clamped rather than rejected. `error` is the only source, and it is where an app puts its own rule (\"image must be at least 400×400\", a rejected upload, a failed encode). A truthy `error` sets `aria-invalid` on the wrapper, which rings the crop area in destructive colors, and renders the message in a `role=\"alert\"` paragraph below the sliders, linked by `aria-describedby`. Should the wrapper already carry an `aria-describedby` of yours, the error id is added to it instead of taking its place. With `showErrorMessage={false}` the ring stays and the message is dropped. `getCroppedImage` clamps bad areas for the same reason, so it never throws on them. It rejects on exactly three things: the image failing to load (\"Could not load image: {src}\"), no 2D canvas context (\"Canvas 2D context is unavailable\"), and the canvas failing to encode the result (\"Could not encode the cropped image\"). The realistic failure is a cross-origin `src` served without CORS headers. Because the image is requested with `crossOrigin=\"anonymous\"`, such a response fails the load outright rather than tainting the canvas, so it surfaces as the load rejection. Worth catching and reporting through `error`.",
}
