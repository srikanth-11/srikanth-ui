...
export function Strength({ password }: { password: string }) {
  ...
  errorState:
    "The field ships no password policy of its own. Nothing is invalid until `validate` says so. It runs on blur with the current value, and whatever it returns becomes the message. A truthy `error` prop marks the field invalid too, and passing `error` at all (even `null`) takes precedence over the internal result. That is how a server-side or form-library error replaces the local one. Either way the input gets `aria-invalid`, the message renders in a `role=\"alert\"` paragraph below and `aria-describedby` links the two. Pass your own `aria-describedby` as well and both ids end up on the input, because the attribute takes a list. With `showErrorMessage={false}` the styling stays and the rendered message is dropped. `onErrorChange` fires only for the internal error, as it appears and clears. `PasswordStrength` is purely informative, and an unmet rule is never an error state.",
}
